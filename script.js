...
/* Show receipt */
function showReceipt({ lines, amount, amountClass, verdict, justification, footerNote }) {
  ...
  // Footer
  if (footerNote) document.getElementById('receipt-footer').innerHTML = footerNote + '<br>GIRL MATH IS REAL MATH<br>— ✦ —';
  ...
  setTimeout(() => {
    ...
    // Scroll to receipt
    setTimeout(() => receipt.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 100);
...
function calculate() {
  ...
  let lines = [];
  let amount = '$0.00', amountClass = 'is-free', verdict = 'FREE', just = '', footerNote = '';

  lines.push(addLine('Date', receiptDate()));
  lines.push(addLine('Scenario', document.getElementById('scenario').options[document.getElementById('scenario').selectedIndex].text));
  ...
  if (scenario === 'treat') {
    ...
    const payLabels = { card:'Card', cash:'Cash (physical)', giftcard:'Gift Card', venmo:'Venmo/CashApp', points:'Reward Points', found:'Found Money' };
    lines.push(addLine('Payment Method', payLabels[payment]));
    if (shipping > 0) lines.push(addLine('Shipping', fmt(shipping)));

    if (price === 0) {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'COMPLIMENTARY';
      just = pick(['It was literally zero dollars. You were simply taking up valuable shelf space in the store.','The item costs nothing. The real cost was the restraint you showed in not buying three.','Free. Gratis. On the house. Stop reading this receipt and go enjoy it.']);
    } else if (['points','found','giftcard'].includes(payment)) {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'FREE MONEY™';
      just = pick({
        points: ['Reward points are imaginary money earned from real spending. You have completed the cycle.', 'Points aren\'t real dollars. They are vibes that someone assigned a number to. Cost: zero.'],
        found:  ['Money found is money gifted by the universe. It had a destiny, and that destiny is this.', 'The sidewalk / couch / old jacket paid for it. You are simply a vessel.'],
        giftcard:['Gift card money is Monopoly money with better vibes. It never existed in your real account.','Someone gave you a gift card because they love you. This purchase honours that love.'],
      }[payment]);
    } else if (payment === 'cash') {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'CASH = FREE';
      just = pick(['Cash doesn\'t appear on any bank statement, therefore it did not financially happen.','Physical bills exist outside the digital banking system. Off the books. Legally complimentary.','You paid in cash. There is no record of this transaction. It was always free.']);
    } else if (payment === 'venmo') {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'VENMO BALANCE™';
      just = pick(['Venmo balance is money that already left your account in the past. This purchase is historical.','Someone owed you money. They paid you on Venmo. You spent that. Net change to your finances: nothing.','The Venmo balance isn\'t \'real\' money until you transfer it. You intercepted it mid-journey. Free.']);
    } else if (shipping > 0 && shipping >= price) {
      amount = fmt(price + shipping); amountClass = ''; verdict = 'RED FLAG';
      just = 'Shipping costs as much as the item. Cancel this order. Add something to your cart to hit the free shipping threshold. We need to be smarter than this.';
      ...
      amount = fmt(price + shipping); amountClass = ''; verdict = 'CORRECTABLE';
      ...
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'NECESSITY';
      just = pick(['Items under $5 fall below the financial visibility threshold. This rounds to a human right.','Under $5 is entry-level spending. The coffee you bought before calculating this cost more.','$'+ price.toFixed(2) +' is not a purchase. It\'s a rounding error on your tax return. Irrelevant.']);
    } else {
  ...
  //  SALE 
    ...
    if (orig <= price && price > 0) {
      amount = fmt(price); amountClass = ''; verdict = 'FULL PRICE PIONEER';
      just = pick(['No sale? You got it before it sold out. Exclusivity pricing. A collector\'s premium.','You paid full price, which means you are the tastemaker who moved first. That\'s worth something.']);
    } else {
      const saved = orig - price;
      lines.push(addLine('You Saved', '+'+fmt(saved)));
      amount = '+'+fmt(saved); amountClass = 'is-profit'; verdict = 'PROFIT';
  ...
  //  COST PER WEAR 
    ...
    if (!wears) {
      amount = fmt(price); amountClass = ''; verdict = 'ARCHIVE PIECE';
      just = pick(['Zero wears means it\'s a display item. Reclassify as home décor and stop feeling guilty.','You\'re not going to wear it. It\'s a collectible now. Some things are art.']);
    } else if (cpw < 1) {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'PENNIES PER WEAR';
      just = pick(['The cost per wear is under one dollar. At this price-per-use you are actively losing money by NOT wearing it more.','Fractions of a cent per use means this item has negative cost. You broke economics.','Under a dollar per wear is a utility rate that rivals your electricity bill. Better value, cuter outfit.']);
    } else if (cpw < 5) {
      amount = fmt(cpw)+' / wear'; amountClass = 'is-profit'; verdict = 'EXCEPTIONAL ROI';
      just = pick([`${fmt(cpw)} per wear is cheaper than a single use of most disposable products. Immaculate value.`,`At ${fmt(cpw)} per occasion, this is statistically the best financial decision in your wardrobe.`]);
    } else {
  ...
  //  PRICE PER DAY 
    ...
    if (!days) {
      amount = fmt(price); amountClass = ''; verdict = 'NEW ACQUISITION';
      just = 'Enter days owned to unlock the full justification. The number will be very small. That\'s the point.';
    } else if (ppd < 0.10) {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'HERITAGE PIECE';
      just = pick([`${fmt(ppd)} per day. This is cheaper than breathing in a major city. Certified free.`,`At ${fmt(ppd)}/day, the item has already paid for itself in vibes approximately ${Math.round(days/30)} months ago.`,`The daily rate has collapsed to nothing. You own a classic now. It's priceless.`]);
    } else if (ppd < 1) {
      amount = fmt(ppd)+' / day'; amountClass = 'is-profit'; verdict = 'DEEPLY REASONABLE';
      ...
      amount = fmt(ppd)+' / day'; amountClass = ''; verdict = 'KEEP USING IT';
  ...
  //  SPLIT 
    ...
    if (people < 2) {
      amount = fmt(price); amountClass = ''; verdict = 'SOLO';
      just = 'You\'re splitting with yourself? That\'s called paying full price. Add friends. This is a group activity.';
    } else if (each < 5) {
      amount = '$0.00'; amountClass = 'is-free'; verdict = 'BELOW THRESHOLD';
      just = pick([`${fmt(each)} each is not a real number. It doesn\'t even register as a purchase. This is communal joy, and communal joy is free.`,`When you divide ${fmt(price)} among ${people} beautiful people, the result is ${fmt(each)}. Which rounds to nothing.`]);
    } else {
